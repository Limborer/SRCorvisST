...
Isigma      = 0.1/255; % default 1/255 for noise-free case. It should be larger than true noise level.
...
if exist(fullfile(folderTestCur,[Iname,'_kernel.png']),'file')
    ...
    % case 2
    kernelsigma = 1; % width (sigma) of the Gaussian blur kernel from [0.6 2.4], e.g., sf = 2, kernelsigma = 1; sf = 3, kernelsigma = 1.6; sf = 4, kernelsigma = 2;
...
ns          = [ns(1)-1,ns];
lamda       = (Isigma^2)/1; % default 3, ****** from {1 2 3 4} ******
...
for itern = 1:totalIter
    ...
    output = output - im;
    imshow(output); title(num2str(itern))
    drawnow;
    % pause(1)
end
...
%%
subplot(1,2,1); imshow(LR,[]); subplot(1,2,2); imshow(output,[]);
return
%%
% return
if showResult
